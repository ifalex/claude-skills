# helm-chart-creator — determinism tests

Verifies the `helm-chart-creator` skill produces the **same chart from the same
interview answers** — on **any** agent harness (Claude Code, GitHub Copilot CLI,
Gemini CLI, Codex). These scripts live in the repo; generated chart output is
written outside it (see `.gitignore`).
